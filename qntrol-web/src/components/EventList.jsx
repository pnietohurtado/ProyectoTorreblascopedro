...
const EventList = ({ events, onEditEvent, onCreateClick }) => {
  ...
  const handleEditClick = (event) => {
    ...
    // Parsear la fecha y hora del evento si existen
    if (event.date) {
      // Asumiendo que event.date es algo como "15/12/2023 - 18:30h"
    ...
    setFileName(event.guestList ? `Lista cargada (${event.guestList.length} invitados)` : '');
  };
      ...
      try {
        ...
        if (data.length > 0) {
          ...
          for(let i = 0; i < data.length; i++){
            sendAlumnoData(data[i].Escaneo, data[i].Nombre, data[i].numInvitados, data[i].id_evento,title, address, date, time);
          }
          // ========================================
        }
      } catch (error) {
  ...
  const handleSubmit = () => {
    ...
    const updatedEvent = {
      ...editingEvent, // Mantener el ID y otros datos originales
      title,
      address,
      date: formattedDate,
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {event.guestList && (
                    <span className="text-xs text-[#7738B0]">
                      {event.guestList.length} invitados
                    </span>
                  )}
                </div>
              </div>
